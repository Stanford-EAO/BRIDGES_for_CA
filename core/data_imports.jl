...
# retirement by 2030: force them to build no new nuclear
if techScenario_Nuclear == "2030"
    ...
    # test: increase geothermal
    primeMover2compare = "Geothermal"
    idx_geothermal = Generators[!, "Fuel"] .== fill(primeMover2compare, size(Generators[!, 8],1), size(Generators[!, 8],2))    
    ...
    Generators[idx_geothermal, 9] = vec( fill(10, size(Generators[idx_geothermal, 9],1), size(Generators[idx_geothermal, 9],2)) )
end
#
...
for i = 1:T_inv
    for g = 1:GEN
        ...
        index = findall(in(subset),scen)
        index = scen[index[1]]
        FuelCosts[i,g] = FuelCostLookup[index, Int(Years[i]-2017)]
    end
    for d = 1:P2G
        ...
        index = findall(in(subset),scen)
        index = scen[index[1]]
        CAPEX_P2G[i,d] = CAPEXLookup[index, Int(Years[i]-2017)]
        subset = findall(in([PrimeMover_P2G[d]]),FOMLookup.Technology)
        scen = findall(in([PrimeMover_P2G[d]]),CostScenarios.Technology)
        scen = findall(in([CostScenarios.Cost[scen[1]]]),FOMLookup.Cost)
        index = findall(in(subset),scen)
        index = scen[index[1]]
        FOM_P2G[i,d] = FOMLookup[index, Int(Years[i]-2017)]
        subset = findall(in([PrimeMover_P2G[d]]),VOMLookup.Technology)
        scen = findall(in([PrimeMover_P2G[d]]),CostScenarios.Technology)
        scen = findall(in([CostScenarios.Cost[scen[1]]]),VOMLookup.Cost)
        index = findall(in(subset),scen)
        index = scen[index[1]]
        VOM_P2G[i,d] = VOMLookup[index, Int(Years[i]-2017)]
    end
    for s = 1:STORAGE_ELEC
        ...
        index = findall(in(subset),scen)
        index = scen[index[1]]
        CAPEX_STORAGE_ELEC[i,s] = CAPEXLookup[index, Int(Years[i]-2017)]
        subset = findall(in([PrimeMover_STORAGE_ELEC[s]]),FOMLookup.Technology)
        scen = findall(in([PrimeMover_STORAGE_ELEC[s]]),CostScenarios.Technology)
        scen = findall(in([CostScenarios.Cost[scen[1]]]),FOMLookup.Cost)
        index = findall(in(subset),scen)
        index = scen[index[1]]
        FOM_STORAGE_ELEC[i,s] = FOMLookup[index, Int(Years[i]-2017)]
    end
    for a = 1:APPLIANCES
        ...
        index = findall(in(subset),scen)
        index = scen[index[1]]
        CAPEX_APPLIANCES[i,a] = CAPEXLookup[index, Int(Years[i]-2017)]
