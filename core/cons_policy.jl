...
@constraint(m, [I = 1:T_inv], excess_powerEmissions[I] <= maxOffsets_elec[I]*(sum(weights[I,T]*8760/t_ops*sum(sum((generation[I,T,t,g]*HeatRate[g] + StartupFuel[g]*startup_GEN[I,T,t,g])*emissions_factors[g]/1000 for g = 1:GEN) for t = 1:t_ops) for T = 1:T_ops)))
...
@constraint(m, [I = 1:T_inv], sum((sum(weights[I,T]*8760/t_ops*sum((generation[I,T,t,g]*HeatRate[g] + StartupFuel[g]*startup_GEN[I,T,t,g]) for t = 1:t_ops) for T = 1:T_ops) - CleanGas_GEN[I, g]/MWh_PER_MMBTU) *emissions_factors[g]/1000 for g = 1:GEN) <= EI_ElecSector[I]/1000*sum(weights[I,T]*8760/t_ops*sum(sum(generation[I,T,t,g0] for g0 = 1:GEN) for t = 1:t_ops) for T = 1:T_ops) + excess_powerEmissions[I])
