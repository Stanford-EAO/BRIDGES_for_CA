...
for i = 1:T_inv
    ...
    # Gen Capex
    # Explanation:                                                        [1 if generator is still in its lifetime, 0 if retired   ]   
    ...
    # Emissions of electricity and gas sector (not emission intensity as above)
...
for i = 1:T_inv
     ...
     GenerationSave[i,GEN+P2G+5] = sum(weights[i,T]*8760/t_ops*sum(sum(InitialAppliancePopulation[:].*ApplianceProfiles_ELEC[T,t,:]) + sum(InitialTransportPopulation[:].*TransportProfiles_ELEC[T,t,:]) + sum(BaselineDemand_ELEC[i,T,t,:])  for t = 1:t_ops) for T = 1:T_ops)
...
TransportDecisions = zeros(5*T_inv,TRANSPORTS)
for i = 1:T_inv
    count = (5*i-2)
    ...
    if i = 1
        TransportDecisions[count+3,:] = InitialTransportPopulation[tr]/1000 - unitsremaining_Transport_historical[i,tr])  # Legacy units failing
        TransportDecisions[count+4,:] = 0   # Vehicles build in previous periods and failing in I. (But there are no previous periods).
    elseif i > 1    
...
ExcessPowerEmissions = JuMP.value.(excess_powerEmissions[:])
...
CSV.write("$(top_dir)/EXCESS_POWER_EMISSIONS.csv",Tables.table(ExcessPowerEmissions'), writeheader = true)
